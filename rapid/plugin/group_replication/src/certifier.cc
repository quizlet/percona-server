...
void Certifier_broadcast_thread::dispatcher()
{
  ...
  int broadcast_period= BROADCAST_PERIOD;
  DBUG_EXECUTE_IF("group_replication_broadcast_time", { broadcast_period= 600; });
  ...
  while (!aborted)
  {
    ...
    if (aborted)
      break;
    set_timespec(&abstime, broadcast_period);
...
int Certifier_broadcast_thread::broadcast_gtid_executed()
{
  ...
  */
  ...
  Gcs_message msg(origin,
                  new Gcs_message_data(0, encoded_gtid_executed_message.size()));
  msg.get_message_data().append_to_payload(&encoded_gtid_executed_message.front(),
                                           encoded_gtid_executed_message.size());

  int send_err= gcs_communication->send_message(msg);
  if (send_err == GCS_MESSAGE_TOO_BIG)
  {
    log_message(MY_ERROR_LEVEL, "Broadcast of committed transactions message "
                                "failed. Message is too big.");
    error= 1;
  }
...
int Certifier::initialize_server_gtid_set(bool get_server_gtid_retrieved)
{
  ...
  if (group_sid.parse(group_name_var) != RETURN_STATUS_OK)
  {
    log_message(MY_ERROR_LEVEL,
                ...
                " the Certification module initialization");
    error= 1;
    goto end;
  }
                           ...
                           const char *member_uuid)
{
  ...
  */
    ...
    */
    if (snapshot_version->ensure_sidno(group_sidno) != RETURN_STATUS_OK)
    {
      log_message(MY_ERROR_LEVEL,
                  ...
                  " transaction being positively certified");
      goto end;
    }
  ...
  /*
    DDL does not have write-set.
  */
  if (write_set->size() > 0)
  {
    Gtid_set_ref *snapshot_version_value=
        new Gtid_set_ref(certification_info_sid_map);
...
void Certifier::garbage_collect()
{
  ...
  */
  if (certifier_garbage_collection_block)
  {
    ...
    my_sleep((broadcast_thread->BROADCAST_PERIOD) * 1000000 * 1.5);
  }
#endif
  ...
  */
  if (channel_add_executed_gtids_to_received_gtids(applier_module_channel_name))
  {
    log_message(MY_WARNING_LEVEL,
                ...
                "on the long run this may cause performance issues");
  }
  ...
  if (this->get_members_size() != plugin_get_group_members_number())
  {
    ...
    */
    ...
    else
    {
      ...
      */
      log_message(MY_WARNING_LEVEL, "The member with address %s has "
                  "already sent the stable set. Therefore discarding the second "
                  "message.", member_id.c_str());
    }
...
rpl_gno Certifier::get_and_increment_sequence_number()
{
  DBUG_ENTER("Certifier::get_and_increment_sequence_number");
      ...
      it != cert_info->end(); ++it)
  {
    ...
    */
    if (it->first.compare(GTID_EXTRACTED_NAME) == 0)
    {
      ...
      {
        log_message(MY_ERROR_LEVEL,
                    "Error reading group_gtid_extracted from the View_change_log_event");
        mysql_mutex_unlock(&LOCK_certification_info);
        DBUG_RETURN(1);
      }
      continue;
    ...
    Gtid_set_ref *value = new Gtid_set_ref(certification_info_sid_map);
...
void
Gtid_Executed_Message::encode_payload(std::vector<unsigned char>* buffer)
{
