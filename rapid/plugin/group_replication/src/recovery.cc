...
void Recovery_module::notify_group_recovery_end()
{
  ...
  Gcs_message msg(origin, new Gcs_message_data(0, encoded_recovery_msg.size()));
  msg.get_message_data().append_to_payload(&encoded_recovery_msg.front(),
                                            encoded_recovery_msg.size());

  enum_gcs_error msg_error= gcs_communication->send_message(msg);
  if (msg_error != GCS_OK)
  {
    log_message(MY_ERROR_LEVEL,
                "Error while sending message for group replication recovery");
