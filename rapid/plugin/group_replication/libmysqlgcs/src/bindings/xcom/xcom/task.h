...
#define ADD_X_EV(when, group_id, msgno, node, from, op, file, state, what) add_xcom_event(when, group_id, msgno, node, from, op, file, state, what)
...
#define ADD_X_EV(when, group_id, msgno, node, from, op, file, state, what)
...
#define TASK_BEGIN                                                      \
  /*   assert(ep);   */                                                 \
  ADD_T_EV(task_now(), __FILE__, stack->sp->state?stack->sp->state:__LINE__, stack->name); TASK_DEBUG("TASK_BEGIN");                             \
...
#define TASK_END                                                        \
  ADD_T_EV(task_now(), __FILE__, stack->sp->state?stack->sp->state:__LINE__, "TASK_END"); TASK_DEBUG("TASK_END");                                                \
  stack->sp->state = 0;                                                 \
...
struct task_event{
  task_event_printer print;
	double when;
	char const *file;
	int state;
	char const *what;
	uint32_t group_id;
	unsigned long long msgno;
	unsigned long node;
	unsigned long from;
	int op;
};
...
extern int connect_tcp(char *server, int port, int *ret);
extern result announce_tcp(int port);
extern int accept_tcp(int fd, int *ret);


extern int task_read(connection_descriptor const* con, void *buf, size_t n, ssize_t *ret);
extern int task_write(connection_descriptor const* con, void *buf, int n, ssize_t *ret);
