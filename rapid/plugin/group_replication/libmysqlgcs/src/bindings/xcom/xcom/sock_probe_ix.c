...
static int init_sock_probe(sock_probe *s)
{
  ...
  {
    if (!(s->ifbuf= (char*)realloc(s->ifbuf, bufsize)))
    {
      abrt= TRUE;
      /* Out of memory. */
      goto err;
       ...
       i++)
  {
    ...
    if (i==ifrpsize || i==0)
    {
      ifrpsize+= IFRP_INIT_ARR_SIZE * sizeof(struct ifreq *);
      /* allocate one more block */
      if (!(s->ifrp= (struct ifreq **) realloc(s->ifrp, ifrpsize)))
      {
        abrt= TRUE;
        /* Out of memory. */
        goto err;
