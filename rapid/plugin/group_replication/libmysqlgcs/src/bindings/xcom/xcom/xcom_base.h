...
typedef int (*port_matcher)(int if_port);
...
#define XCOM_FSM(action, arg) do {const char *s = xcom_state_name[xcom_fsm(action,arg)]; G_MESSAGE("%f %s:%d new state %s",seconds(), __FILE__, __LINE__, s);} while(0)
