...
/* Match any port */
static int yes(int port MY_ATTRIBUTE((unused)))
{
  return 1;
}

/* Create tasks and enter the task main loop */
int	taskmain(int listen_port)
{
...
/* Create tasks and enter the task main loop */
int	xcom_taskmain(int listen_port)
{
	init_xcom_transport(listen_port);
...
int	xcom_taskmain2(int listen_port)
{
  init_xcom_transport(listen_port);
...
{
	ADD_X_EV(seconds(), a->app_key.group_id, a->app_key.msgno, a->app_key.node, a->app_key.node, 0,__FILE__, __LINE__, "install_node_group");
	if(a)
	...
	while (!xcom_shutdown) { /* Loop until no more work to do */
		...
		if(!is_config(ep->client_msg->p->a->body.c_t)){
		    while(AUTOBATCH &&
				...
				atmp = tmp->p->a;
				/* Abort batching if config */
				if(is_config(atmp->body.c_t)){
...
site_def *handle_add_node(app_data_ptr a)
{
	...
	MAY_DBG(FN; COPY_AND_FREE_GOUT(dbg_list(&a->body.app_u_u.nodes)); );
	ADD_X_EV(seconds(), a->app_key.group_id, a->app_key.msgno, a->app_key.node, a->app_key.node, 0,__FILE__, __LINE__, "handle_add_node");
	assert(get_site_def());
	assert(site);
...
site_def *handle_remove_node(app_data_ptr a)
{
	site_def * site = clone_site_def(get_site_def());
	DBGOUT(FN; COPY_AND_FREE_GOUT(dbg_list(&a->body.app_u_u.nodes)));
	ADD_X_EV(seconds(), a->app_key.group_id, a->app_key.msgno, a->app_key.node, get_nodeno(site), 0, __FILE__, __LINE__,
	     "handle_remove_node");
	...
	while (!xcom_shutdown) {
		for (; ; ) {
			if (ep->state == FETCH) {
				...
				ADD_X_EV(task_now(), ep->p->synode.group_id, ep->p->synode.msgno, ep->p->synode.node, 0, 0, __FILE__, __LINE__, "executor executing message");
...
#endif
		ADD_X_EV(task_now(),executed_msg.group_id,executed_msg.msgno,executed_msg.node,666,0, __FILE__, __LINE__, "sweeper ready");
...
/*                         SYCEXP(executed_msg); SYCEXP(max_synode); SYCEXP(ep->find));  */
		 {
			while (synode_lt(ep->find, max_synode) && ! too_far(ep->find)) {
				...
				pax_machine * pm = 0;
				ADD_X_EV(task_now(),ep->find.group_id,ep->find.msgno,ep->find.node,667,0, __FILE__, __LINE__, "sweeper examining");
				...
				if (pm && !pm->force_delivery) { /* We want full 3 phase Paxos for forced messages */
					...
					if (!is_busy_machine(pm) && pm->acceptor.promise.cnt == 0 && ! pm->acceptor.msg && !finished(pm)) {
						pm->op = skip_op;
						ADD_X_EV(task_now(),ep->find.group_id,ep->find.msgno,ep->find.node,668,pm->op, __FILE__, __LINE__, "sweeper skipping");
...
#else
		if(get_nodeno(site) == VOID_NODE_NO)
			send_to_others(get_site_def(), pm, "send_read");
		else
...
{
	ADD_X_EV(seconds(), p->synode.group_id, p->synode.msgno, p->synode.node, pm->from, pm->op,__FILE__, __LINE__, "handle_prepare");
#if 0
	DBGOUT(FN;
...
{
	ADD_X_EV(seconds(), p->synode.group_id, p->synode.msgno, p->synode.node, m->from, m->op,__FILE__, __LINE__, "do_learn");
...
{
	ADD_X_EV(seconds(), p->synode.group_id, p->synode.msgno, p->synode.node, m->from, m->op,__FILE__, __LINE__, "handle_ack_prepare");
#if 0
	DBGOUT(FN;
	    NDBG(pm->from, d); NDBG(pm->to, d);
	    SYCEXP(pm->synode);
...
{
	ADD_X_EV(seconds(), p->synode.group_id, p->synode.msgno, p->synode.node, m->from, m->op,__FILE__, __LINE__, "handle_ack_prepare_empty");
#if 0
	DBGOUT(FN;
	    NDBG(pm->from, d); NDBG(pm->to, d);
	    SYCEXP(pm->synode);
...
static void	handle_accept(site_def const * site, pax_machine *p, linkage *reply_queue, pax_msg *m)
{
	...
	ADD_X_EV(seconds(), p->synode.group_id, p->synode.msgno, p->synode.node, m->from, m->op,__FILE__, __LINE__, "handle_accept");
...
{
	ADD_X_EV(seconds(), p->synode.group_id, p->synode.msgno, p->synode.node, m->from, m->op,__FILE__, __LINE__, "handle_ack_accept");
...
{
	if (sweeper) {
		ADD_X_EV(task_now(), max_synode.group_id, max_synode.msgno, max_synode.node, 669, 0, __FILE__, __LINE__, "sweeper activated");
...
pax_msg *dispatch_op(site_def const *site, pax_msg *p, linkage *reply_queue)
{
	...
	ADD_X_EV(task_now(), p->synode.group_id, p->synode.msgno, p->synode.node, p->from, p->op, __FILE__, __LINE__, "dispatch_op");
	switch (p->op) {
	case client_msg:
		clicnt++;
...
int	acceptor_learner_task(task_arg arg)
{
	...
	if (xcom_use_ssl()) {
		...
		{
			int	ret_ssl = SSL_accept(ep->rfd.ssl_fd);
			int	err = SSL_get_error(ep->rfd.ssl_fd, ret_ssl);
	...
	while (!xcom_shutdown) {
		...
		if(use_buffered_read){
			...
			TASK_CALL(read_msg(&ep->rfd, ep->p, &n));
                ...
                if ((ep->p->op < client_msg || ep->p->op > LAST_OP))
                {
...
int	reply_handler_task(task_arg arg)
{
	...
	for (; ; ) {
		...
		}
		{
			...
			receive_bytes[ep->reply->op] += n + MSG_HDR_SIZE;
		}
		MAY_DBG(FN;
...
void	xcom_send_data(u_int size, char *data)
{
...
void send_client_app_data(char *srv, int port, app_data_ptr a)
{
...
void send_client_boot(char *srv, int port, node_list *nl)
{
	send_client_app_data(srv, port, create_config(nl, unified_boot_type));
}

void send_client_add_node(char *srv, int port, node_list *nl)
{
	send_client_app_data(srv, port, create_config(nl, add_node_type));
}

void send_client_remove_node(char *srv, int port, node_list *nl)
{
	send_client_app_data(srv, port, create_config(nl, remove_node_type));
}

void send_client_config(char *srv, int port, node_list *nl)
{
...
void xcom_add_node(char *addr, int port, node_list *nl)
{
...
{
	int	xcom_port = xcom_get_port(addr);
	char	*xcom_addr = xcom_get_name(addr);

	if (xcom_mynode_match(xcom_addr, xcom_port)) {
		node_list x_nl;
...
static int	init_sockaddr(char *server, struct sockaddr *sock_addr, socklen_t *sock_size, int port)
{
...
static int	socket_read_bytes(connection_descriptor *rfd, char *p, int n)
{
	int	left;
	char	*bytes;
	...
	bytes = (char *)p;
	while (left > 0) {
		nread = socket_read(rfd, bytes, left);
		if (nread.val == 0) {
			return 0;
		} else if (nread.val < 0) {
			return - 1;
...
static int	socket_write(connection_descriptor *wfd, void *_buf, int n)
{
	char	*buf = (char*) _buf;
	result 	ret = {0,0};

	int	total; /* Keeps track of number of bytes written so far */

	assert(n >= 0);
	total = 0;
	while (total < n) {
		while ((ret = con_write(wfd, buf + total, n - total)).val < 0 &&
		...
		if (ret.val <= 0) { /* Something went wrong */
			...
			total += ret.val; /* Add number of bytes written to total */
		}
	}
	DBGOUT(FN; NEXP(total, d); NEXP(n, d));
	assert(total == n);
	return(total);
}
...
static int timed_connect(int fd, sockaddr *sock_addr, socklen_t sock_size)
{
  ...
  if (res < 0)
  {
    if (errno == EINPROGRESS)
    {
      ...
      {
        G_WARNING("select - Error while connecting: %d",
                  WSAGetLastError());
#else
      else if (res < 0)
      {
        G_WARNING("select - Error while connecting: %d - %s",
      ...
      else
      {
        ...
        if (FD_ISSET(fd, &efds))
        {
          G_WARNING("select - Check socket for errors!");
          return -1;
        }
      }
    }
    ...
    {
#if defined (WIN32) || defined (WIN64)
      G_WARNING("connect - Error connecting %d", WSAGetLastError());
#else
      G_WARNING("connect - Error connecting %d - %s",
                errno, strerror(errno));
#endif
      return -1;
    }
...
static connection_descriptor*	connect_xcom(char *server, int port)
{
	result fd = {0,0};
	result ret = {0,0};
	...
	SET_OS_ERR(0);
	if (timed_connect(fd.val, &sock_addr, sock_size) == -1) {
		G_MESSAGE("Error connecting to server %s %d.", server, port);
...

	{
		...
		SET_OS_ERR(0);
		ret.val = peer = getpeername(fd.val, &sock_addr, &sock_size);
		...
		if (peer >= 0) {
			...
			G_MESSAGE("connected to %s %d", server, port);
		} else {
			/* Something is wrong */
		...
		if (xcom_use_ssl()) {
			...
			DBGOUT(FN; STRLIT("ssl connected"); NDBG(fd.val, d); PTREXP(ssl));
...
connection_descriptor*	xcom_open_client_connection(char *server, int port)
{
...
static int	xcom_send_proto(connection_descriptor *con, xcom_proto x_proto, x_msg_type x_type, unsigned int tag)
{
	...
	if (con->fd >= 0) {
		...
		{
			int	sent;
			sent = socket_write(con, buf, MSG_HDR_SIZE);
			if (con->fd < 0) {
				return -1;
			}
			return sent;
...
static int	xcom_send_client_app_data(connection_descriptor *fd, app_data_ptr a, int force)
{
	pax_msg * msg = pax_msg_new(null_synode, 0);
	u_int	sent = 0;
	u_int	buflen;
	...
	if(! proto_done(fd)){
		...
		x_msg_type x_type;
		unsigned int tag;
		int n = xcom_send_proto(fd, my_xcom_version, x_version_req, TAG_START);
		G_DEBUG("client sent negotiation request for protocol %d",my_xcom_version);
		if(n < 0)
			return n;
		n = xcom_recv_proto(fd, &x_proto, &x_type, &tag);
		if(n < 0)
			return n;
		if(tag != TAG_START)
			return -1;
	...
	if(buflen){
		sent = socket_write(fd, buf, buflen);
		if (buflen != sent) {
			DBGOUT(FN; STRLIT("write failed "); NDBG(fd->fd, d);
				   NDBG(buflen, d); NDBG(sent, d));
		}
		X_FREE(buf);
	}
...
int	xcom_client_send_data(u_int size, char *data, connection_descriptor *fd)
{
	app_data a;
	int retval = 0;
	init_app_data(&a);
...
int	xcom_client_boot(connection_descriptor *fd, node_list *nl, uint32_t group_id)
{
	app_data a;
	int retval = 0;
	retval =  xcom_send_client_app_data(fd, init_config_with_group(&a, nl, unified_boot_type, group_id), 0);
	my_xdr_free((xdrproc_t) xdr_app_data, (char*)&a);
	return retval;
}
...
int xcom_send_app_wait(connection_descriptor *fd, app_data *a, int force)
{
	...
	for(;;){
		retval = xcom_send_client_app_data(fd, a, force);
		if(retval < 0)
			return 0;
		memset(&p, 0, sizeof(p));
