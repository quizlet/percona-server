...
void *task_allocate(task_env *p, unsigned int bytes)
{
	...
	unsigned int	alloc_units = (bytes + sizeof(TaskAlign) -1) / sizeof(TaskAlign);
	TaskAlign * ret;
...
static unsigned int	send_bytes;
static unsigned int	receive_bytes;
...
int	task_read(connection_descriptor const* con, void *buf, size_t n, ssize_t *ret)
{
	DECL_ENV
		int dummy;
	END_ENV;
...
int	task_write(connection_descriptor const *con, void *_buf, int n, ssize_t *ret)
{
	char	*buf = (char *) _buf;
	DECL_ENV
	...
	TASK_BEGIN
	    ep->total = 0;
	while (ep->total < n) {
		MAY_DBG(FN; PTREXP(stack); NDBG(con->fd,d); NDBG(n - ep->total,d));
		for(;;){
			sock_ret = con_write(con, buf + ep->total, n - ep->total);
			*ret = sock_ret.val;
			task_dump_err(sock_ret.funerr);
			MAY_DBG(FN; PTREXP(stack); NDBG(con->fd,d); NDBG(*ret,d));
		...
		if (0 == *ret) { /* We have successfully written n bytes */
			TERMINATE;
		} else if (*ret < 0) { /* Something went wrong */
			TASK_FAIL;
		} else {
			ep->total += *ret; /* Add number of bytes written to total */
		}
	}
	assert(ep->total == n);
...
int	connect_tcp(char *server, int port, int *ret)
{
	DECL_ENV
	    int	fd;
	...
	if (!init_sockaddr(server, &ep->sock_addr, &ep->sock_size, port)) {
...
static void init_server_addr(struct sockaddr *sock_addr, int port)
{
...
result	announce_tcp(int port)
{
	...
	if (bind(fd.val, &sock_addr, sizeof(sock_addr)) < 0) {
		goto err;
	}
...
void x_ev_print(task_event *te MY_ATTRIBUTE((unused)))
{
	G_DEBUG("{%x %llu %lu} %lu op %s",
	    te->group_id,
	    (long long unsigned int)te->msgno,
	    (long unsigned int)te->node,
	    te->from,
	    pax_op_to_str(te->op));
}


void t_ev_print(task_event *te MY_ATTRIBUTE((unused)))
{
	/* G_DEBUG(NEWLINE); */
}


void w_ev_print(task_event *te MY_ATTRIBUTE((unused)))
{
	G_DEBUG("ms %d",
	    te->op);
}
...
static void add_base_event(double when, char const *file, int state, char const *what)
{
	if (max_task_event == MAX_TASK_EVENT)
		return;
	task_events[cur_task_event].when = when;
	task_events[cur_task_event].file = file;
	task_events[cur_task_event].state = state;
	task_events[cur_task_event].what = what;
...
void add_task_event(double when, char const *file, int state, char const *what)
{
	if (max_task_event == MAX_TASK_EVENT)
		return;
	task_events[cur_task_event].print = t_ev_print;
	;
	add_base_event(when, file, state, what);
}


void add_xcom_event(double when, uint32_t group_id, unsigned long long msgno,
                    unsigned long node, unsigned long from, int op,
                    char const *file, int state, char const *what)
{
	if (max_task_event == MAX_TASK_EVENT)
		return;

	task_events[cur_task_event].print = x_ev_print;
	;

	task_events[cur_task_event].group_id = group_id;
	task_events[cur_task_event].msgno = msgno;
	task_events[cur_task_event].node = node;
	task_events[cur_task_event].from = from;
	task_events[cur_task_event].op = op;
...
void add_wait_event(double when, char *file, int state, char *what, int milli)
{
	...
	task_events[cur_task_event].print = w_ev_print;

	task_events[cur_task_event].op = milli;
