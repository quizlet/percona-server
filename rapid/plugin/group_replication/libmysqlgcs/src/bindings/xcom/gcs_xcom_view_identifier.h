...
{
public:
  explicit Gcs_xcom_view_identifier(unsigned long int fixed_part_arg,
                                    int monotonic_part_arg);

  virtual ~Gcs_xcom_view_identifier();


  unsigned long int get_fixed_part() const
  {
    return fixed_part;
  }
...
private:
  void init(unsigned long int fixed_part_arg, int monotonic_part_arg);
