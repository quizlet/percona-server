...
static const int XCOM_MAX_HANDLERS= 20;
...
std::string *Gcs_xcom_utils::build_xcom_member_id(std::string &address)
{
...
Gcs_xcom_proxy_impl::xcom_client_open_connection(std::string saddr, int port)
{
...
xcom_client_add_node(connection_descriptor* fd, node_list *nl, unsigned long gid)
{
  return ::xcom_client_add_node(fd, nl, gid);
}

int
Gcs_xcom_proxy_impl::xcom_client_remove_node(connection_descriptor* fd, node_list* nl, unsigned long gid)
{
...
int
Gcs_xcom_proxy_impl::xcom_client_remove_node(node_list *nl, unsigned long gid)
{
...
int Gcs_xcom_proxy_impl::xcom_client_boot(node_list *nl, unsigned long gid)
{
  int index= xcom_acquire_handler();
  int res=   true;
    ...
    if (index != -1)
    {
      ...
      */
      if (fd != NULL)
      {
        assert(len > 0);
        int written=
          ::xcom_client_send_data(static_cast<unsigned int>(len), data, fd);
...
int Gcs_xcom_proxy_impl::xcom_init(int xcom_listen_port)
{
  /* Init XCom */
...
{
  return ::xcom_init_ssl(server_key_file, server_cert_file, client_key_file,
                         client_cert_file, ca_file, ca_path, crl_file,
                         crl_path, cipher, tls_version);
}
...
bool Gcs_xcom_proxy_impl::xcom_open_handlers(std::string saddr, int port)
{
  bool success= true;
  char *addr=   (char *)saddr.c_str();
  int n=        0;
  ...
  if (m_xcom_handlers_cursor == -1 && port != -1 && addr != NULL)
  {
