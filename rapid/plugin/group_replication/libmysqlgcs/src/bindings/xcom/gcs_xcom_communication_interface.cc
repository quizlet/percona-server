...
{
  /*
    If the node has not installed a view yet although it may have
    received a global view message, it has to buffer any message
    that it may get and deliver them when the view is installed.

    In other words, the node is executing the view exchange phase
    and cannot deliver any data message before installing the view.
    Members of the group that have installed an old view don't need
    to buffer messages. In the future, we may decide to make them
    buffer such messages in order to provide a flow control when
    the state exchange is happening.
...
void Gcs_xcom_communication::buffer_message(Gcs_message *message)
{
  ...
  assert(!m_view_control->belongs_to_group());
  assert(m_view_control->is_view_changing());
}
...
void Gcs_xcom_communication::deliver_buffered_messages()
{
       ...
       buffer_msg_it++)
  {
    ...
    /*
      This is called after the view being delivered. See the
      following method: Gcs_xcom_control::install_view().
    */
    assert(m_view_control->belongs_to_group());
    assert(!m_view_control->is_view_changing());
  }
