...
public:
  ...
  */
  ...
  /**
    ...
    @param[in] nodes      A mask representing the nodes in the group that are
                          online
    @param[in] same_view  Whether this global view was already delivered.
  */

  bool xcom_receive_global_view(synode_no message_id, node_set nodes, bool
                                same_view);


  bool xcom_receive_local_view(synode_no message_id, node_set nodes);
...
private:
   ...
   */
  void build_total_members(const site_def &local_site_definition,
                           node_set &nodes,
