...
static void *xcom_taskmain_startup(void *ptr)
{
  ...
  int port= gcs_ctrl->get_local_member_info()->get_member_port();
...
build_total_members(const site_def &local_site_definition,
                    node_set &nodes,
                    ...
                    std::vector<Gcs_member_identifier *> &failed_members)
{
  for (unsigned int i= 0; i < nodes.node_set_len; i++)
  {
    ...
    */
    string address(local_site_definition.nodes.node_list_val[i].address);
    string *member_id_str= Gcs_xcom_utils::build_xcom_member_id(address);
...
bool
Gcs_xcom_control::
xcom_receive_local_view(synode_no message_id, node_set nodes)
{
  const site_def *site_def= m_xcom_proxy->find_site_def(message_id);
  ...
  // ignore
  if (nodes.node_set_len <= 0)
    goto end;

  // ignore
  if(site_def == NULL || site_def->nodeno == VOID_NODE_NO)
    goto end;
  ...
  if (current_view != NULL)
  {
    ...
    for (i= 0; i < nodes.node_set_len; i++)
    {
      std::string addr= site_def->nodes.node_list_val[i].address;
      Gcs_member_identifier gcs_id(addr);
...
xcom_receive_global_view(synode_no message_id, node_set nodes,
                         bool same_view)
{
  bool ret= false;
  bool free_built_members= false;
  const site_def *local_site_definition= NULL;
  ...
  */
  ...
  /*
    The cluster configuration, i.e. its nodes, is installed at some point
    in time which is associated to a message number because a consensus
    round is required to change the configuration. Every message carries
    this configuration message number and it is possible to fetch the
    associated configuration using it.

    Some test cases create a mocked proxy which returns null here for
    that reason, we need this branch in the execution path. The nodeno
    may be void if a view is forced after a majority loss and the view
    exclude a node that may be alive. In that case, the alive node will
    get void.
  */
  local_site_definition= m_xcom_proxy->find_site_def(message_id);
  if(local_site_definition == NULL || local_site_definition->nodeno == VOID_NODE_NO)
  {
    MYSQL_GCS_LOG_WARN("Processing new view on handler without a valid" <<
                       " group configuration.")
    ret= true;
    goto end;
  }
  ...
  MYSQL_GCS_LOG_TRACE(
    ...
    << "node_id is " << local_site_definition->nodeno
  )
  ...
  */
  build_total_members(*local_site_definition,
                      nodes,
                      alive_members,
                      failed_members);
  /*
  ...
  */
  ...
  MYSQL_GCS_DEBUG_EXECUTE(
    for(it= alive_members.begin(); it != alive_members.end(); it++)
      MYSQL_GCS_LOG_TRACE(
        "(My node_id is " << local_site_definition->nodeno << ") Node " <<
  ...
  */
  if (same_view || failed_members.size() != 0)
  {
    MYSQL_GCS_LOG_TRACE(
       "(My node_id is " <<
       local_site_definition->nodeno << ") ::xcom_receive_global_view():: "
