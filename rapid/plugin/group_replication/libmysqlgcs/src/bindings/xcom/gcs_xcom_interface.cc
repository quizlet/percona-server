...
void do_cb_xcom_receive_data(synode_no message_id, node_set nodes,
                             u_int size, char *data);
void      cb_xcom_receive_local_view(synode_no message_id, node_set nodes);
void      do_cb_xcom_receive_local_view(synode_no message_id, node_set nodes);
...
initialize_xcom(const Gcs_interface_parameters &interface_params)
{
  ...
  */
  ...
  if (xcom_proxy->xcom_use_ssl())
  {
    ...
    int inited_ssl= xcom_proxy->xcom_init_ssl(
                          ...
                          char *data)
{
  Gcs_xcom_notification *notification=
    new Data_notification(do_cb_xcom_receive_data, message_id, nodes, size, data);
...
void do_cb_xcom_receive_data(synode_no message_id, node_set nodes, u_int size,
                             char *data)
{
  ...
  */
  ...
  const site_def *local_site_definition= find_site_def(message_id);

  MYSQL_GCS_LOG_TRACE(
    "::xcom_receive_data_internal():: xcom_receive_data "
    << " My node_id is " << local_site_definition->nodeno
    << " message_id.group= " << message_id.group_id
    << " message_id.msgno= " << message_id.msgno
    << " message_id.node= "  << message_id.node
  )

  Gcs_member_identifier origin(string(local_site_definition
                                      ->nodes
                                      .node_list_val[message_id.node]
                                      .address));
...
{
  Gcs_xcom_notification *notification=
    new Global_view_notification(do_cb_xcom_receive_global_view, config_id, message_id, nodes);
  bool scheduled= gcs_engine->push(notification);
  if (!scheduled)
  {
    MYSQL_GCS_LOG_DEBUG(
...
void do_cb_xcom_receive_global_view(synode_no config_id, synode_no message_id, node_set nodes)
{
  Gcs_xcom_interface *intf=
    static_cast<Gcs_xcom_interface *>(Gcs_xcom_interface::get_interface());
  ...
  */
  ...
  MYSQL_GCS_DEBUG_EXECUTE(
    ...
    {
      MYSQL_GCS_LOG_TRACE("My node_id is " << local_site_definition->nodeno
                          << " peer: " << i
                          << " address: "
                          << local_site_definition->servers[i]->srv
                          << ":"
                          << local_site_definition->servers[i]->port
                          << " flag: "
                          << (nodes.node_set_val[i]?"Active": "Failed"))
    }
  )
  ...
  */
  ...
  if(!(xcom_control_if->xcom_receive_global_view(message_id, nodes, same_view)))
  {
...
int cb_xcom_match_port(int if_port)
{
  return xcom_local_port == if_port;
}


void cb_xcom_receive_local_view(synode_no message_id MY_ATTRIBUTE((unused)),
                                node_set nodes)
{
  Gcs_xcom_notification *notification=
    new Local_view_notification(do_cb_xcom_receive_local_view, message_id, nodes);
  bool scheduled= gcs_engine->push(notification);
  if (!scheduled)
  {
    MYSQL_GCS_LOG_DEBUG(
...
void do_cb_xcom_receive_local_view(synode_no message_id, node_set nodes)
{
  ...
  xcom_ctrl->xcom_receive_local_view(message_id, nodes);
